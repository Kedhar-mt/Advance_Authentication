...
exports.login = async (req, res) => {
    ...
    try {
      ...
      await User.findByIdAndUpdate(user._id, { 
        isLoggedIn: true,
        deviceInfo: { userAgent, ip, lastLogin: new Date() }
      });
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    try {
      const bytes = CryptoJS.AES.decrypt(newPassword, process.env.SECRET_KEY);
      const decryptedPassword = bytes.toString(CryptoJS.enc.Utf8);
      ...
      return res.status(500).json({ msg: "Error decrypting password" });
    }
